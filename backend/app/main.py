...
async def solar_proxy(
    ...
    """Proxy a request to the configured solar data provider."""

    try:
        payload = await fetch_solar_data(settings, lat=lat, lon=lon, date=date)
...
async def traffic_proxy(
    station_id: str = Query(..., description="Road traffic station identifier"),
    date: str = Query(..., description="ISO date"),
    settings: Settings = Depends(get_settings),
) -> dict[str, object]:
    """Proxy a request to the configured traffic provider."""

    try:
        payload = await fetch_traffic_data(settings, station_id=station_id, date=date)
    except ExternalServiceError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
...
async def weather_proxy(
    ...
    """Proxy a request to the configured weather provider."""

    try:
        payload = await fetch_weather_data(settings, station_id=station_id, date=date)
    except ExternalServiceError as exc:
        raise HTTPException(status_code=502, detail=str(exc)) from exc
